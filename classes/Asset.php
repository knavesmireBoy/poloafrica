...
class Asset
{
  ...
  */
  ...
  protected $filename = null;
    ...
    public function __construct($articleID/*, Article $article*/) {
        ...
        $this->setPathPropertiesFromQuery();
     }
    
    protected function setPathPropertiesFromUpload($asset){
        $this->filename = strtolower(explode('.', trim($asset['name']))[0]);
        $this->extension = strtolower(strrchr($asset['name'], '.'));
    }    
    
    protected function setPathPropertiesFromQuery(){
        $conn = getConn();
  ...
  */
  public function storeUploadedFile($asset) {
    if ( $asset['error'] == UPLOAD_ERR_OK )
    {
      ...
      if ( is_null($this->articleID) ) {
        ...
        $this->setPathPropertiesFromUpload($asset);
        $this->insert();
        $this->validate($asset);
  ...
  public function getFilePath($type = IMG_TYPE_FULLSIZE) {
    ...
    // Insert the Image
    $conn = getConn();
    $sql = "INSERT INTO assets (extension, name) VALUES (:extension, :name)";
    $st = $conn->prepare($sql);
